...
expr <- fread('GSE42127.gene_expression_array.tsv.gz', data.table = F)
...
survival <- fread('GSE42127.clinical.tsv.gz', data.table = F)
colnames(survival) <- c('ID', 'Patient', 'Tissue', 'time', 'status')
row.names(survival) <- survival$ID
...
# Warning: The following features are not present in the object: MEX3A, AK4, LINC00665, not searching for symbol synonyms
