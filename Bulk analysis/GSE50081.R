...
gset <- getGEO('GSE50081', destdir=".", AnnotGPL = F, getGPL = F)
gset <- gset[[1]]
expr <- exprs(gset)
...
# clinic
pheno <- pData(gset)
pheno <- subset(pheno, subset = pheno$characteristics_ch1.1 == 'histology: adenocarcinoma')
survival <- pheno[, c(10, 15, 16, 18, 19)]
...
survival <- survival[, -1]
colnames(survival) <- c('Sex', 'Stage', 'Age', 'time', 'status')

survival$Sex <- gsub(survival$Sex, pattern = 'Sex: ', replacement = '')
survival$Stage <- gsub(survival$Stage, pattern = 'Stage: ', replacement = '')
survival$status <- gsub(survival$status, pattern = 'status: ', replacement = '')
survival$time <- gsub(survival$time, pattern = 'survival time: ', replacement = '')
survival$Age <- gsub(survival$Age, pattern = 'age: ', replacement = '')

survival$status <- ifelse(survival$status == 'alive', 0, 1)
...
survival <- fread('survival_matrix.txt', data.table = F)

survival$time <- as.numeric(survival$time)
survival$time <- 365 * survival$time

num <- mean(survival$CDKN2A)
survival$CDKN2A <- ifelse(survival$CDKN2A >= num, 'high', 'low')

num <- mean(survival$DLD)
survival$DLD <- ifelse(survival$DLD >= num, 'high', 'low')

num <- mean(survival$FDX1)
...
estimate <- function(tpm, project){
  ...
  write.table(tpm, file = input.f, sep = '\t', quote = F)
